...
class ClientConnection:
    ...
    last_action: float
    action_count: int
    uid: Optional[int] = None  # For multi-tenancy
...
class WebSocketServer:
    ...
    async def handle_websocket(self, request: web.Request) -> web.WebSocketResponse:
        ...
        logger.info(f"WebSocket connected: {request.remote} (total: {self.stats['current_connections']})")
        ...
        try:
            ...
            logger.info(f"WebSocket disconnected (remaining: {self.stats['current_connections']})")
    ...
    async def handle_subscribe(
        ...
        if len(client.channels) + len(channels) > self.max_subscriptions:
            await self.send_error(
                ws,
                f"Max subscriptions exceeded ({self.max_subscriptions})"
            )
            return
    ...
    async def handle_cancel_job(
        ...
        try:
            async with self.db_pool.acquire() as conn:
                ...
                if result == "UPDATE 0":
                    await self.send_error(ws, f"Job {job_id} not found or cannot be cancelled")
                else:
                    await self.send_to_client(ws, {
                        'event': 'job_cancelled',
                        'timestamp': datetime.utcnow().isoformat(),
        ...
        try:
            async with self.db_pool.acquire() as conn:
                ...
                if new_job_id:
                    ...
                    await self.send_error(ws, f"Job {job_id} not found or cannot be retried")
    ...
    async def handle_adjust_priority(
        ...
        try:
            async with self.db_pool.acquire() as conn:
                ...
                if result == "UPDATE 0":
                    await self.send_error(ws, f"Job {job_id} not found or cannot adjust priority")
                else:
                    await self.send_to_client(ws, {
                    ...
                    logger.info(f"Job {job_id} priority adjusted to {new_priority} via WebSocket")
        ...
        while True:
            try:
                ...
                for queue, stats in queue_stats.items():
                    await self.broadcast_event(f'queues:{queue}', {
                        'event': 'queue_stats',
                        'timestamp': datetime.utcnow().isoformat(),
                        'data': {
                            'queue': queue,
    ...
    async def start(self, host: str = '0.0.0.0', port: int = 8082):
        ...
        async def health_check(request):
            return web.json_response({
                'status': 'healthy',
                'stats': self.stats,
                'notify_connection': not self.notify_conn.is_closed() if self.notify_conn else False,
                'timestamp': datetime.utcnow().isoformat()
            })
...
async def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python -m pyjobby.websocket_server <config_file> [--host HOST] [--port PORT]")
        sys.exit(1)
