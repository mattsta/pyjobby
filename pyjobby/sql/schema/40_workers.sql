...
CREATE TABLE jorb_worker (
    ...
    -- --app-version`); NULL means it runs unversioned work only
    app_version  TEXT,
    version      TEXT,
...
COMMENT ON COLUMN jorb_worker.app_version IS 'The APPLICATION code version this worker advertises (`pj --app-version`, else the config file''s app_version, else NULL). Published for exactly the reason capabilities and max_prio are: it is a third thing that decides what this worker will claim, it lives in this process and nowhere else, and without it in the registry "does any live worker on this queue run version X?" is unanswerable -- which is the same silence that made a job above every ceiling invisible. NOT the same column as jorb_worker.version, which is the pyjobby LIBRARY version this process imported: that one says which platform is running, this one says whose application code is, and a deployment upgrades them on completely different schedules. claim_jorb() compares it to jorb.app_version, and the comparison is one-sided -- a versioned worker claims unversioned work as happily as an unversioned one does, because the JOB decides whether it is pinned (see jorb.app_version). Written once at registration and never rewritten, like max_prio: the version a process runs is a startup constant, and a worker on new code is a new process and therefore a new row.';
