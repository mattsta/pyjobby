...
class TestScheduleSafetyManagerIntegration:
    ...
    async def test_check_concurrency_empty(self, db_pool):
        ...
        async with db_pool.acquire() as conn:
            ...
            is_safe, count = await manager.check_concurrency(schedule_id=99999, max_concurrent=5)
    ...
    async def test_check_concurrency_under_limit(self, db_pool):
        ...
        async with db_pool.acquire() as conn:
            ...
            manager = ScheduleSafetyManager(conn)
            is_safe, count = await manager.check_concurrency(schedule_id=schedule_id, max_concurrent=5)
            assert is_safe is True
            assert count == 1
    ...
    async def test_check_concurrency_at_limit(self, db_pool):
        ...
        async with db_pool.acquire() as conn:
            ...
            manager = ScheduleSafetyManager(conn)
            is_safe, count = await manager.check_concurrency(schedule_id=schedule_id, max_concurrent=3)
    ...
    async def test_check_backpressure_no_threshold(self, db_pool):
        ...
        async with db_pool.acquire() as conn:
            manager = ScheduleSafetyManager(conn)
            is_safe, depth = await manager.check_backpressure(queue='test_queue', threshold=None)
            assert is_safe is True
            assert depth == 0
    ...
    async def test_check_backpressure_over_threshold(self, db_pool):
        ...
        async with db_pool.acquire() as conn:
            ...
            manager = ScheduleSafetyManager(conn)
            is_safe, depth = await manager.check_backpressure(queue=queue_name, threshold=3)
            assert is_safe is False
            assert depth == 5
    ...
    async def test_check_circuit_breaker_safe(self, db_pool):
        ...
        async with db_pool.acquire() as conn:
            schedule = {'id': 1, 'name': 'safe_schedule', 'consecutive_failures': 2, 'circuit_breaker_threshold': 5}
            manager = ScheduleSafetyManager(conn)
    ...
    async def test_check_circuit_breaker_triggered(self, db_pool):
        ...
        async with db_pool.acquire() as conn:
            ...
            schedule = {'id': schedule_id, 'name': name, 'consecutive_failures': 5, 'circuit_breaker_threshold': 5}
            manager = ScheduleSafetyManager(conn)
            is_safe, reason = await manager.check_circuit_breaker(schedule)
            assert is_safe is False
...
class TestScheduleManagerIntegration:
    ...
    async def test_update_schedule_next_run(self, db_pool):
        ...
        async with db_pool.acquire() as conn:
            ...
            manager = ScheduleManager(conn)
            try:
                await manager.update_schedule_next_run(schedule_id=schedule_id, cron_expr='0 * * * *', timezone='UTC')
            except ImportError:
                pytest.skip("croniter not installed")
    ...
    async def test_record_execution_success(self, db_pool):
        ...
        async with db_pool.acquire() as conn:
            ...
            row = await conn.fetchrow("SELECT run_count, success_count, consecutive_failures FROM jorb_schedule WHERE id = $1", schedule_id)
    ...
    async def test_record_execution_failure(self, db_pool):
        ...
        async with db_pool.acquire() as conn:
            ...
            row = await conn.fetchrow("SELECT run_count, failure_count, consecutive_failures FROM jorb_schedule WHERE id = $1", schedule_id)
            assert row['run_count'] == 6
    ...
    async def test_record_execution_skip(self, db_pool):
        ...
        async with db_pool.acquire() as conn:
            ...
            row = await conn.fetchrow("SELECT skip_count FROM jorb_schedule WHERE id = $1", schedule_id)
...
class TestSchedulerWorkerIntegration:
    ...
    async def test_create_scheduled_job(self, db_pool):
        ...
        async with db_pool.acquire() as conn:
            schedule = {
                'id': 12345, 'name': 'job_create_test', 'job_class': 'TestJob',
                'kwargs': {}, 'queue': 'default', 'prio': 100, 'capability': None
            }
            worker = SchedulerWorker(conn)
    ...
    async def test_create_scheduled_job_duplicate(self, db_pool):
        ...
        async with db_pool.acquire() as conn:
            schedule = {
                'id': 54321, 'name': 'dup_test', 'job_class': 'TestJob',
                'kwargs': {}, 'queue': 'default', 'prio': 100, 'capability': None
            }
            worker = SchedulerWorker(conn)
    ...
    async def test_log_execution(self, db_pool):
        ...
        async with db_pool.acquire() as conn:
            ...
            job_id = await conn.fetchval("INSERT INTO jorb (job_class, kwargs, state) VALUES ('TestJob', '{}', 'queued') RETURNING id")
            schedule = {'id': schedule_id, 'name': name}
            result = ScheduleExecutionResult(result='success', job_id=job_id, jitter_applied=3, queue_depth=10, concurrent_jobs=1, duration_ms=150)
            worker = SchedulerWorker(conn)
            await worker.log_execution(schedule, datetime.utcnow(), result)
            row = await conn.fetchrow("SELECT * FROM jorb_schedule_log WHERE schedule_id = $1", schedule_id)
    ...
    async def test_execute_schedule_success(self, db_pool):
        ...
        async with db_pool.acquire() as conn:
            ...
            schedule = dict(await conn.fetchrow("SELECT * FROM jorb_schedule WHERE id = $1", schedule_id))
            worker = SchedulerWorker(conn)
