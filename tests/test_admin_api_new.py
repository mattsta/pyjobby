...
class TestAdminAPIJobManagement:
    ...
    async def test_list_jobs_order_by(self, db_pool):
        ...
        async with db_pool.acquire() as conn:
            ...
            jobs_asc = await api.list_jobs(queue=queue, order_by='prio', order_dir='ASC')
            assert jobs_asc[0]['job_class'] == 'LowPrio'

            jobs_desc = await api.list_jobs(queue=queue, order_by='prio', order_dir='DESC')
...
class TestAdminAPIWorkerManagement:
    ...
    async def test_list_workers(self, db_pool):
        ...
        async with db_pool.acquire() as conn:
            ...
            worker = next((w for w in workers if w['worker_host'] == 'host1' and w['worker_pid'] == 12345), None)
    ...
    async def test_worker_stats(self, db_pool):
        ...
        async with db_pool.acquire() as conn:
            ...
            worker = next((w for w in stats['workers'] if w['host'] == 'stats_host'), None)
            if worker:
    ...
    async def test_get_schedule_history_filter_result(self, db_pool):
        ...
        async with db_pool.acquire() as conn:
            ...
            await conn.execute("""
                INSERT INTO jorb_schedule_log (schedule_id, schedule_name, scheduled_time, job_id, result)
                VALUES ($1, $2, NOW(), $3, 'success')
            """, schedule_id, name, job_id1)
            await conn.execute("""
                INSERT INTO jorb_schedule_log (schedule_id, schedule_name, scheduled_time, job_id, result)
                VALUES ($1, $2, NOW(), $3, 'failure')
            """, schedule_id, name, job_id2)

            success_history = await api.get_schedule_history(schedule_id, result_filter='success')
