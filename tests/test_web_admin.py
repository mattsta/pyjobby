...
class TestWebAdminServerInit:
    ...
    def test_routes_setup(self):
        ...
        routes = [r.resource.canonical for r in server.app.router.routes() if hasattr(r, 'resource')]
        assert '/' in routes
...
class TestQueuesAPI:
    ...
    async def test_api_queues_list(self, web_admin_client, db_pool):
        ...
        assert resp.status == 200 or resp.status == 500  # May fail if DB connection issue
...
class TestSchedulesAPI:
    ...
    async def test_api_schedule_create(self, web_admin_client):
        ...
        resp = await web_admin_client.post('/api/schedules', data={
            'name': name,
            'job_class': 'CreateJob',
            'cron_expr': '0 2 * * *',
            'queue': 'test',
