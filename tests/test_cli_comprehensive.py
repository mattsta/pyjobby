...
def mock_db_params():
    ...
    return {'user': 'test', 'password': 'test', 'database': 'test', 'host': 'localhost', 'port': 5432}
...
def mock_admin_api():
    ...
    mock_api.list_jobs.return_value = [
        {'id': 1, 'job_class': 'test.Job', 'state': 'queued', 'queue': 'default',
         'created': datetime.now(), 'prio': 100, 'run_after': None},
        {'id': 2, 'job_class': 'test.Job2', 'state': 'running', 'queue': 'high',
         'created': datetime.now(), 'prio': 200, 'run_after': None},
    ]

    mock_api.get_job_full.return_value = {
        'id': 1, 'job_class': 'test.Job', 'state': 'queued', 'queue': 'default',
        'kwargs': {'key': 'value'}, 'created': datetime.now(), 'prio': 100,
        'error_count': 0, 'error_message': None, 'run_after': None,
    ...
    mock_api.queue_stats.return_value = {
        'queued': 10, 'running': 5, 'finished': 100, 'crashed': 2,
        'waiting': 1, 'total': 118, 'oldest_queued': datetime.now() - timedelta(hours=2)
    }

    mock_api.list_workers.return_value = [
        {'queue': 'default', 'capability': 'std', 'last_heartbeat': datetime.now(), 'active_count': 3},
        {'queue': 'high', 'capability': 'gpu', 'last_heartbeat': datetime.now(), 'active_count': 1},
    ]

    mock_api.worker_stats.return_value = {
        'total_workers': 2, 'total_active': 4, 'queues': ['default', 'high']
    }

    mock_api.list_dlq.return_value = [
        {'id': 10, 'job_class': 'test.FailedJob', 'state': 'crashed', 'error_message': 'test error',
         'error_count': 10, 'created': datetime.now()},
    ]

    mock_api.retry_from_dlq.return_value = None

    mock_api.get_execution_metrics.return_value = {
        'total_jobs': 1000, 'success_rate': 95.5, 'avg_duration': 12.3
    }

    mock_api.list_schedules.return_value = [
        {'schedule_id': 1, 'name': 'test-schedule', 'job_class': 'test.Job',
         'cron_expr': '0 * * * *', 'enabled': True, 'queue': 'default'},
    ]

    mock_api.get_schedule.return_value = {
        'schedule_id': 1, 'name': 'test-schedule', 'job_class': 'test.Job',
        'cron_expr': '0 * * * *', 'enabled': True, 'queue': 'default',
        'kwargs': {}, 'next_run': datetime.now() + timedelta(hours=1)
    }
    ...
    mock_api.get_schedule_stats.return_value = [
        {'name': 'test-schedule', 'success_count': 95, 'failure_count': 5, 'success_rate_pct': 95.0},
    ]
...
class TestJobsCommands:
    ...
    def test_jobs_list_default(self, cli_runner, mock_admin_api):
        ...
        mock_db_params = {'user': 'test', 'password': 'test', 'database': 'test', 'host': 'localhost', 'port': 5432}
        ...
        with patch('pyjobby.cli.asyncpg.connect', new=AsyncMock()):
            with patch('pyjobby.cli.AdminAPI', return_value=mock_admin_api):
                result = cli_runner.invoke(cli, [
                    '--config', 'nonexistent.py', 'jobs', 'list',
                    '--queue', 'default',
                    '--state', 'queued',
                    '--limit', '10'
                ])
    ...
    def test_jobs_list_json_output(self, cli_runner, mock_admin_api):
        ...
        with patch('pyjobby.cli.asyncpg.connect', new=AsyncMock()):
            with patch('pyjobby.cli.AdminAPI', return_value=mock_admin_api):
                result = cli_runner.invoke(cli, [
                    '--config', 'nonexistent.py', 'jobs', 'list', '--json'
                ])
    ...
    def test_jobs_inspect(self, cli_runner, mock_admin_api):
        ...
        with patch('pyjobby.cli.asyncpg.connect', new=AsyncMock()):
            with patch('pyjobby.cli.AdminAPI', return_value=mock_admin_api):
                result = cli_runner.invoke(cli, [
                    '--config', 'nonexistent.py', 'jobs', 'inspect', '1'
                ])
    ...
    def test_jobs_retry(self, cli_runner, mock_admin_api):
        ...
        with patch('pyjobby.cli.asyncpg.connect', new=AsyncMock()):
            with patch('pyjobby.cli.AdminAPI', return_value=mock_admin_api):
                result = cli_runner.invoke(cli, [
                    '--config', 'nonexistent.py', 'jobs', 'retry', '1', '2'
                ])
    ...
    def test_jobs_cancel(self, cli_runner, mock_admin_api):
        ...
        with patch('pyjobby.cli.asyncpg.connect', new=AsyncMock()):
            with patch('pyjobby.cli.AdminAPI', return_value=mock_admin_api):
                result = cli_runner.invoke(cli, [
                    '--config', 'nonexistent.py', 'jobs', 'cancel', '1'
                ])
...
class TestQueuesCommands:
    ...
    def test_queues_list(self, cli_runner, mock_admin_api):
        ...
        with patch('pyjobby.cli.asyncpg.connect', new=AsyncMock()):
            with patch('pyjobby.cli.AdminAPI', return_value=mock_admin_api):
                result = cli_runner.invoke(cli, [
                    '--config', 'nonexistent.py', 'queues', 'list'
                ])
    ...
    def test_queues_stats(self, cli_runner, mock_admin_api):
        ...
        with patch('pyjobby.cli.asyncpg.connect', new=AsyncMock()):
            with patch('pyjobby.cli.AdminAPI', return_value=mock_admin_api):
                result = cli_runner.invoke(cli, [
                    '--config', 'nonexistent.py', 'queues', 'stats'
                ])
    ...
    def test_queues_stats_json(self, cli_runner, mock_admin_api):
        ...
        with patch('pyjobby.cli.asyncpg.connect', new=AsyncMock()):
            with patch('pyjobby.cli.AdminAPI', return_value=mock_admin_api):
                result = cli_runner.invoke(cli, [
                    '--config', 'nonexistent.py', 'queues', 'stats', '--json'
                ])
    ...
    def test_queues_clear_with_force(self, cli_runner, mock_admin_api):
        ...
        with patch('pyjobby.cli.asyncpg.connect', new=AsyncMock()):
            with patch('pyjobby.cli.AdminAPI', return_value=mock_admin_api):
                result = cli_runner.invoke(cli, [
                    '--config', 'nonexistent.py', 'queues', 'clear', 'default', '--force'
                ])
...
class TestWorkersCommands:
    ...
    def test_workers_list(self, cli_runner, mock_admin_api):
        ...
        with patch('pyjobby.cli.asyncpg.connect', new=AsyncMock()):
            with patch('pyjobby.cli.AdminAPI', return_value=mock_admin_api):
                result = cli_runner.invoke(cli, [
                    '--config', 'nonexistent.py', 'workers', 'list'
                ])
    ...
    def test_workers_stats(self, cli_runner, mock_admin_api):
        ...
        with patch('pyjobby.cli.asyncpg.connect', new=AsyncMock()):
            with patch('pyjobby.cli.AdminAPI', return_value=mock_admin_api):
                result = cli_runner.invoke(cli, [
                    '--config', 'nonexistent.py', 'workers', 'stats'
                ])
...
class TestDLQCommands:
    ...
    def test_dlq_list(self, cli_runner, mock_admin_api):
        ...
        with patch('pyjobby.cli.asyncpg.connect', new=AsyncMock()):
            with patch('pyjobby.cli.AdminAPI', return_value=mock_admin_api):
                result = cli_runner.invoke(cli, [
                    '--config', 'nonexistent.py', 'dlq', 'list'
                ])
    ...
    def test_dlq_retry(self, cli_runner, mock_admin_api):
        ...
        with patch('pyjobby.cli.asyncpg.connect', new=AsyncMock()):
            with patch('pyjobby.cli.AdminAPI', return_value=mock_admin_api):
                result = cli_runner.invoke(cli, [
                    '--config', 'nonexistent.py', 'dlq', 'retry', '10'
                ])
...
class TestMetricsCommands:
    ...
    def test_metrics_json(self, cli_runner, mock_admin_api):
        ...
        with patch('pyjobby.cli.asyncpg.connect', new=AsyncMock()):
            with patch('pyjobby.cli.AdminAPI', return_value=mock_admin_api):
                result = cli_runner.invoke(cli, [
                    '--config', 'nonexistent.py', 'metrics', '--json'
                ])
...
class TestScheduleCommands:
    ...
    def test_schedule_list(self, cli_runner, mock_admin_api):
        ...
        with patch('pyjobby.cli.asyncpg.connect', new=AsyncMock()):
            with patch('pyjobby.cli.AdminAPI', return_value=mock_admin_api):
                result = cli_runner.invoke(cli, [
                    '--config', 'nonexistent.py', 'schedule', 'list'
                ])
    ...
    def test_schedule_add(self, cli_runner, mock_admin_api):
        ...
        with patch('pyjobby.cli.asyncpg.connect', new=AsyncMock()):
            with patch('pyjobby.cli.AdminAPI', return_value=mock_admin_api):
                result = cli_runner.invoke(cli, [
                    '--config', 'nonexistent.py', 'schedule', 'add',
                    'test-schedule', 'test.Job', '0 * * * *'
                ])
    ...
    def test_schedule_enable(self, cli_runner, mock_admin_api):
        ...
        with patch('pyjobby.cli.asyncpg.connect', new=AsyncMock()):
            with patch('pyjobby.cli.AdminAPI', return_value=mock_admin_api):
                result = cli_runner.invoke(cli, [
                    '--config', 'nonexistent.py', 'schedule', 'enable', 'test-schedule'
                ])
    ...
    def test_schedule_disable(self, cli_runner, mock_admin_api):
        ...
        with patch('pyjobby.cli.asyncpg.connect', new=AsyncMock()):
            with patch('pyjobby.cli.AdminAPI', return_value=mock_admin_api):
                result = cli_runner.invoke(cli, [
                    '--config', 'nonexistent.py', 'schedule', 'disable', 'test-schedule'
                ])
    ...
    def test_schedule_delete_with_force(self, cli_runner, mock_admin_api):
        ...
        with patch('pyjobby.cli.asyncpg.connect', new=AsyncMock()):
            with patch('pyjobby.cli.AdminAPI', return_value=mock_admin_api):
                result = cli_runner.invoke(cli, [
                    '--config', 'nonexistent.py', 'schedule', 'delete', 'test-schedule', '--force'
                ])
    ...
    def test_schedule_history(self, cli_runner, mock_admin_api):
        ...
        with patch('pyjobby.cli.asyncpg.connect', new=AsyncMock()):
            with patch('pyjobby.cli.AdminAPI', return_value=mock_admin_api):
                result = cli_runner.invoke(cli, [
                    '--config', 'nonexistent.py', 'schedule', 'history', 'test-schedule'
                ])
    ...
    def test_schedule_stats(self, cli_runner, mock_admin_api):
        ...
        with patch('pyjobby.cli.asyncpg.connect', new=AsyncMock()):
            with patch('pyjobby.cli.AdminAPI', return_value=mock_admin_api):
                result = cli_runner.invoke(cli, [
                    '--config', 'nonexistent.py', 'schedule', 'stats'
                ])
...
class TestCLIErrorHandling:
    ...
    def test_invalid_job_id(self, cli_runner, mock_admin_api):
        ...
        with patch('pyjobby.cli.asyncpg.connect', new=AsyncMock()):
            with patch('pyjobby.cli.AdminAPI', return_value=mock_admin_api):
                result = cli_runner.invoke(cli, [
                    '--config', 'nonexistent.py', 'jobs', 'inspect', '99999'
                ])
