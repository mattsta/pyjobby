...
class DataTransformJob:
    ...
    def __init__(self, upstream_result: Optional[dict] = None, operation: str = "double", **kwargs):
...
class TestDAGExecution:
    ...
    async def test_linear_dag_execution(self, db_pool):
        ...
        job2_id = await db_pool.fetchval("""
            ...
            RETURNING id
        """, "tests.test_e2e_producer_consumer.SimpleTestJob",
            {"step": 2}, "e2e_test", "waiting", dag_id, job1_id)
        ...
        job3_id = await db_pool.fetchval("""
            ...
            RETURNING id
        """, "tests.test_e2e_producer_consumer.SimpleTestJob",
            {"step": 3}, "e2e_test", "waiting", dag_id, job2_id)
        ...
        await db_pool.execute("""
            ...
            SET state = 'finished',
                ...
                finished = $3
            WHERE id = $1
        """, job1_id, {"step": 1, "status": "done"}, datetime.now(timezone.utc))
    ...
    async def test_parallel_dag_execution(self, db_pool):
        ...
        job1_id = await db_pool.fetchval("""
            ...
            RETURNING id
        """, "tests.test_e2e_producer_consumer.SimpleTestJob",
            {"branch": "A"}, "e2e_test", "queued", dag_id)
        ...
        job2_id = await db_pool.fetchval("""
            ...
            RETURNING id
        """, "tests.test_e2e_producer_consumer.SimpleTestJob",
            {"branch": "B"}, "e2e_test", "queued", dag_id)
        ...
        job3_id = await db_pool.fetchval("""
            ...
            RETURNING id
        """, "tests.test_e2e_producer_consumer.SimpleTestJob",
            {"merge": True}, "e2e_test", "waiting", dag_id)
        ...
        assert len(deps) == 2
        assert set([d['depends_on_job_id'] for d in deps]) == {job1_id, job2_id}

        # Cleanup
        await db_pool.execute("DELETE FROM jorb_dependencies WHERE job_id = $1", job3_id)
        await db_pool.execute("DELETE FROM jorb WHERE dag_id = $1", dag_id)
        await db_pool.execute("DELETE FROM jorb_dag WHERE id = $1", dag_id)
...
class TestResultPassing:
    ...
    async def test_result_passed_to_downstream_job(self, db_pool):
        ...
        """, "tests.test_e2e_producer_consumer.DataTransformJob",
            {"initial_data": [1, 2, 3]}, "e2e_test", "queued")
        ...
        await db_pool.execute("""
            ...
            SET state = 'finished',
                ...
                finished = $3
            WHERE id = $1
        """, job1_id, result1, datetime.now(timezone.utc))
        ...
        job2_id = await db_pool.fetchval("""
            ...
            RETURNING id
        """, "tests.test_e2e_producer_consumer.DataTransformJob",
            {"operation": "double"}, "e2e_test", "waiting", job1_id)
