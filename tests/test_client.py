...
class TestJobClientEnqueue:
    ...
    async def test_enqueue_with_waitfor_job(self, db_pool):
        ...
        async with db_pool.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT state, waitfor_job FROM jorb WHERE id = $1", job2_id
            )
    ...
    async def test_enqueue_with_retry_strategy(self, db_pool):
        ...
        job_id = await client.enqueue(
            "RetryJob", retry_strategy="linear", max_retries=5, initial_retry_delay=10
        )

        async with db_pool.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT admin_data FROM jorb WHERE id = $1", job_id
            )
    ...
    async def test_enqueue_with_timeout(self, db_pool):
        ...
        job_id = await client.enqueue(
            "TimeoutJob", timeout_seconds=30, on_timeout="fail"
        )
...
class TestJobClientAdvancedFeatures:
    ...
    async def test_create_pipeline(self, db_pool):
        ...
        async with db_pool.acquire() as conn:
            row2 = await conn.fetchrow(
                "SELECT waitfor_job FROM jorb WHERE id = $1", job_ids[1]
            )
    ...
    async def test_create_fan_out(self, db_pool):
        ...
        async with db_pool.acquire() as conn:
            for jid in job_ids:
                row = await conn.fetchrow(
                    "SELECT run_group FROM jorb WHERE id = $1", jid
                )
