...
class TestRetryDelayProperties:
    ...
    @given(
        config_data=st.fixed_dictionaries({
            'retry_strategy': st.sampled_from(['exponential', 'linear', 'fibonacci', 'fixed', None]),
            'max_retries': st.one_of(st.none(), st.integers(min_value=1, max_value=50)),
            'initial_retry_delay': st.one_of(st.none(), st.integers(min_value=1, max_value=60)),
            'max_retry_delay': st.one_of(st.none(), st.integers(min_value=60, max_value=7200))
        })
    )
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
    def test_get_retry_config_always_returns_valid_config(self, config_data):
        ...
        assert config['retry_strategy'] in ['exponential', 'linear', 'fibonacci', 'fixed']
...
class TestDAGProperties:
    ...
    def test_fan_out_fan_in_has_three_levels(self, root_count, leaf_count):
        ...
        middles = [dag.add(f"Middle{i}", {}, depends_on=roots) for i in range(leaf_count)]
...
class TestResultStorageProperties:
    ...
    def test_result_is_json_serializable(self, result_data):
        ...
        assert type(decoded) in (type(result_data), dict, list, str, int, float, bool, type(None))

    @given(
        result_size=st.integers(min_value=0, max_value=5000)
    )
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
...
class TestEdgeCases:
    ...
    @given(
        initial_delay=st.integers(min_value=0, max_value=0)
    )
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
