...
class TestDAGWaitForJob:
    ...
    async def test_waitfor_job_triggers_dependent_jobs(self, db_pool, db_params):
        ...
        async with db_pool.acquire() as conn:
            parent_job = await conn.fetchrow(
                "SELECT * FROM jorb WHERE id = $1", parent_id
            )
            child_job = await conn.fetchrow(
                "SELECT * FROM jorb WHERE id = $1", child_id
            )

            # Parent should be finished
            assert parent_job["state"] == "finished", (
                f"Parent state: {parent_job['state']}"
            )
            assert parent_job["result"] == "result: parent"

            # Child should also be finished (triggered after parent completed)
            assert child_job["state"] == "finished", (
                f"Child state: {child_job['state']}"
            )
    ...
    async def test_waitfor_job_with_multiple_dependent_jobs(self, db_pool, db_params):
        ...
        async with db_pool.acquire() as conn:
            ...
            for i, child_id in enumerate(child_ids):
                ...
                assert child_job["state"] == "finished", (
                    f"Child {i} state: {child_job['state']}"
                )
...
class TestDAGWaitForGroup:
        ...
        async with db_pool.acquire() as conn:
            ...
            for i, job_id in enumerate(group_job_ids):
                ...
                assert job["state"] == "finished", (
                    f"Group job {i} state: {job['state']}"
                )

            # Waiting job should be triggered and finished
            waiting_job = await conn.fetchrow(
                "SELECT * FROM jorb WHERE id = $1", waiting_job_id
            )
            assert waiting_job["state"] == "finished", (
                f"Waiting job state: {waiting_job['state']}"
            )
    ...
    async def test_waitfor_group_not_triggered_until_all_finish(
        ...
        async with db_pool.acquire() as conn:
            ...
            assert job1["state"] == "finished", (
                f"Job1 should finish, got: {job1['state']}"
            )
            assert job2["state"] == "crashed", (
                f"Job2 should be crashed, got: {job2['state']}"
            )
